                                    ...
                                    @foreach ($piece->elements as $element)
                                        <div class="border border-slate-200 rounded-lg p-4 bg-white">
                                            ...
                                            <form method="POST" action="{{ route('elements.update', $element) }}"
                                                ...
                                                <div>
                                                    <label class="block text-xs text-slate-500 mb-1">Observations (les
                                                        références photos s'ajoutent automatiquement)</label>
                                                    <div class="flex gap-2">
                                                        <input type="text" name="observations"
                                                            value="{{ $element->observations }}"
                                                            placeholder="Ex: Légère rayure, trace d'usure..."
                            ...
                            <div class="border-t border-slate-200 pt-5">
                                    ...
                                    <div class="grid grid-cols-4 sm:grid-cols-6 gap-3 mb-4">
                                        @foreach ($allPhotos as $index => $photo)
                                            <div class="text-center relative">
                                                <a href="{{ $photo->url }}" data-lightbox="piece-{{ $piece->id }}"
                                                    data-src="{{ $photo->url }}"
                                ...
                                @if ($piece->elements->isNotEmpty())
                                    <div class="bg-slate-50 rounded-lg p-4">
                                        <p class="text-xs text-slate-500 mb-3">Ajouter une photo (la référence sera ajoutée
                                            automatiquement dans les observations)</p>
                                                ...
                                                <label class="flex-1 cursor-pointer">
                                                    <div
                                                        ...
                                                        <span class="text-sm text-slate-600 truncate"
                                                            id="file-label-piece-{{ $piece->id }}">Choisir une
                                                            photo</span>
                                                    </div>
                                                    <input type="file" name="photo" accept="image/*" required
                                                        class="hidden"
                                                        onchange="document.getElementById('file-label-piece-{{ $piece->id }}').textContent = this.files[0]?.name || 'Choisir une photo'">
                                                </label>
