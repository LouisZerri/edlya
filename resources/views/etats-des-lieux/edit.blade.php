            ...
            <div class="bg-white p-6 rounded-lg border border-slate-200">
                                ...
                                <div class="space-y-4 mb-6">
                                    @foreach ($piece->elements as $element)
                                        <div class="border border-slate-200 rounded-lg p-4 bg-white hover:shadow-sm transition-shadow">
                                            ...
                                            <div class="flex items-start justify-between mb-3">
                                                <div>
                                                    ...
                                                    @if ($element->observations)
                                                        <p class="text-sm text-slate-600 mt-2 italic">{{ $element->observations }}</p>
                                                    @endif
                                                </div>
                                                <form method="POST" action="{{ route('elements.destroy', $element) }}" onsubmit="return confirm('Supprimer cet élément et ses photos ?')">
                                                    @csrf
                                                    @method('DELETE')
                                                    ...
                                                    @foreach($element->photos as $photo)
                                                        <div class="relative">
                                                            ...
                                                            <form method="POST" action="{{ route('photos.destroy', $photo) }}" class="absolute -top-2 -right-2">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button type="submit" class="bg-red-500 hover:bg-red-600 text-white rounded-full w-6 h-6 text-sm cursor-pointer flex items-center justify-center shadow-md transition-colors" onclick="return confirm('Supprimer cette photo ?')" title="Supprimer la photo">×</button>
                                                            </form>
                                                            @if($photo->legende)
                                                                <p class="text-xs text-slate-500 mt-1 text-center max-w-24 truncate">{{ $photo->legende }}</p>
                                                            @endif
                                                        </div>
                                                    @endforeach
                                                </div>
                                            ...
                                            <form method="POST" action="{{ route('photos.store', $element) }}" enctype="multipart/form-data" class="pt-3 border-t border-slate-100">
                                                @csrf
                                                <div class="flex items-center gap-3">
                                                    <label class="flex-1 cursor-pointer">
                                                        <div class="flex items-center gap-3 px-4 py-3 bg-slate-50 border border-slate-200 border-dashed rounded-lg hover:bg-slate-100 hover:border-slate-300 transition-colors">
                                                            <svg class="w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" />
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" />
                                                            </svg>
                                                            <span class="text-sm text-slate-600 truncate" id="file-label-{{ $element->id }}">Cliquez pour ajouter une photo</span>
                                                        </div>
                                                        <input type="file" name="photo" accept="image/*" required class="hidden" onchange="document.getElementById('file-label-{{ $element->id }}').textContent = this.files[0]?.name || 'Cliquez pour ajouter une photo'">
                                                    </label>
                                                    <input type="hidden" name="latitude" class="geolocation-lat">
                                                    <input type="hidden" name="longitude" class="geolocation-lng">
                                                    <button type="submit" class="bg-primary-600 text-white px-4 py-3 rounded-lg hover:bg-primary-700 cursor-pointer transition-colors">
                                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @empty
                    <div class="text-center py-12 bg-slate-50 rounded-lg border border-dashed border-slate-300">
                        ...
                        <p class="text-sm text-slate-400">Commencez par ajouter une pièce ci-dessous</p>
                    </div>
                @endforelse
