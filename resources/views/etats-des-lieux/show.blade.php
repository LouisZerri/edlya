    ...
    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-8">
        ...
        <div class="grid grid-cols-2 sm:flex sm:flex-wrap gap-2 sm:gap-3">
            ...
            <form method="POST" action="{{ route('etats-des-lieux.destroy', $etatDesLieux) }}"
                onsubmit="return confirm('Supprimer cet état des lieux et toutes ses données ?')">
                @csrf
                @method('DELETE')
                <button type="submit"
                    class="inline-flex items-center justify-center gap-2 bg-red-600 text-white px-3 sm:px-4 py-2 rounded-lg hover:bg-red-700 transition-colors text-sm font-medium min-h-[44px] w-full cursor-pointer">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
